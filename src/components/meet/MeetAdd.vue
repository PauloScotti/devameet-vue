<script lang="ts">
    ...
    export default defineComponent({
        ...
        components: { MeetAddEditHeaderVue },
        data(){
            return {
                color: '',
                name: ''
        ...
        computed: {
            isFormValid(){
                return this.color && this.color.trim().length > 0 && this.name && this.name.trim().length > 0;
            },
            getFormValidClass(){
                if(!this.color || this.color.trim().length < 4 || !this.name || this.name.trim().length < 2) {
                    return 'disabled'
                }
...
<template>
    <div class="container-principal">
        <div class="container-meet">
            <MeetAddEditHeaderVue :color="color" :name="name" @setColor="setColor" @setName="setName" />
            <div class="actions">
